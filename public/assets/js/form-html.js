// Form Barang HTML Table - AJAX Implementation
            ...
            if (response.data.status && response.data.data.length > 0) {
                response.data.data.forEach(item => {
                    ...
                    row.addEventListener('click', function() {
                        openModalEditHtml(item.id, item.nama_barang, item.harga);
                    });
...
function setupEventListeners() {
    ...
    // Form Enter Key
    const formHtml = document.getElementById('form-barang-html');
    if (formHtml) {
    ...
    if (btnHapus) {
        btnHapus.addEventListener('click', deleteBarangHtml);
    }

    // Modal Save Button
    const btnSimpan = document.getElementById('btn-simpan-html');
    if (btnSimpan) {
        btnSimpan.addEventListener('click', updateBarangHtml);
    }

    // Modal Close Button (Clear Form)
    const modalHtml = document.getElementById('modal-barang-html');
    if (modalHtml) {
...
function submitFormHtml() {
    const namaBarang = document.getElementById('nama_barang_html').value.trim();
    const hargaBarang = document.getElementById('harga_barang_html').value.trim();
    ...
    .then(response => {
        if (response.data.status) {
            ...
            document.getElementById('form-barang-html').reset();
            loadBarangData();
        }
    })
    .catch(error => {
...
// Open Modal Edit
function openModalEditHtml(id, nama, harga) {
    document.getElementById('modal-id-html').value = id;
    document.getElementById('modal-nama-html').value = nama;
    document.getElementById('modal-harga-html').value = harga;
    
    const modal = new bootstrap.Modal(document.getElementById('modal-barang-html'));
    modal.show();
}

// Update Barang
function updateBarangHtml() {
    const id = document.getElementById('modal-id-html').value;
    const nama = document.getElementById('modal-nama-html').value.trim();
    const harga = document.getElementById('modal-harga-html').value.trim();
    ...
    .then(response => {
        if (response.data.status) {
            showAlert('Success', response.data.message, 'success');
            bootstrap.Modal.getInstance(document.getElementById('modal-barang-html')).hide();
            loadBarangData();
        }
    })
    .catch(error => {
